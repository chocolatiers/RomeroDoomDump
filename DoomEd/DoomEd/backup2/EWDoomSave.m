...
//	
// count the number of lines to write out
//
	for (i= 0 ; i<numlines ; i++)
	...
	for (i= 0 ; i<numlines ; i++)
	{
		...
		if (wline->selected != -1)
		{
			...
			for (s=0 ; s<=  ( (wline->flags&ML_TWOSIDED) != 0) ; s++ )
			{
				...
				side->midheight = ShortSwap (wside->midheight);
				...
				{	// validate single sided wall
					...
					if (mtex < 0 || top > wside->midheight 
					|| bottom < wside->midheight - textures[mtex].height)
	...
	for (i=0 ; i<count ; i++)		
	{
		*list_p++ = LongSwap (buf_p-buffer);
		...
		for (j=0 ; j<lcount ; j++)
		{
			...
			msector->lines[j] = ShortSwap(*linenum);
		}
	}
...
- saveDoomMap
{
	...
	if (sectorerror)
	{
		NXRunAlertPanel ("Error","Sector grouping errors",NULL,NULL,NULL);
		return self;
	}
