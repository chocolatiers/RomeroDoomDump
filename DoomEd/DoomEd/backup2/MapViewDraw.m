...
// only draw the lines that might intersect the visible rect
	...
	// add a line to the path for it's type
		if (li->selected)
			...
			color = ONESIDED_C;
		AddLine (color, points[li->p1].pt.x, points[li->p1].pt.y,points[li->p2].pt.x, points[li->p2].pt.y);
...
- drawThings: (const NXRect *)rect
{
	...
	unselected = unsel_p = alloca (numthings*sizeof(NXRect));
	selected = sel_p = alloca (numthings*sizeof(NXRect));
	...
	for ( wp = things ; wp < stop ; wp++)
	{
		...
		if (wp->origin.x <left || wp->origin.x > right || wp->origin.y > top || wp->origin.y < bottom)
			continue;
		...
		use->origin.x = wp->origin.x - offset/2;
		use->origin.y = wp->origin.y - offset/2;
...
//
	...
	if (count)
	{
		NXSetColor ([prefpanel_i colorFor: THING_C]);
